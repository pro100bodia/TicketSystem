...
public class JpaListTicketModelRepositoryTest {
    ...
    @InjectMocks
    private JpaListTicketModelRepository subject;
    ...
    public void givenNoArgs_whenFindAll_ThenReturnTicketModels() {
        ...
        //when
        when(ticketEntityRepository.findAll()).thenReturn(tickets);

        java.lang.reflect.Type targetListType = new TypeToken<List<TicketModel>>() {
        }.getType();
        for (int i = 0; i < ticketModels.size(); i++) {
            when(modelMapper.map(tickets.get(i), TicketModel.class)).thenReturn(ticketModels.get(i));
        }

        List<TicketModel> result = subject.findAll();
        //then
    ...
    public void givenTicketModel_whenAddTicket_thenVerifyTicketIsSaved() {
        ...
        //when
        when(modelMapper.map(ticketModel, Ticket.class)).thenReturn(tickets.get(2));
        subject.addTicket(ticketModel);

        //then
        verify(ticketEntityRepository).save(tickets.get(2));
    }
    ...
    public void givenTicketId_whenDeleteTicket_thenVerifyTicketIsDeleted() {
        ...
        //then
        verify(ticketEntityRepository).deleteById(ticketId);
    }
}